...
Citizen.CreateThread(function()
    ...
    local aztecasmap = AddBlipForCoord(-149.92, -1588.43, 34.8)

    SetBlipSprite(aztecasmap, 310)
    SetBlipColour(aztecasmap, 38)
    SetBlipScale(aztecasmap, 0.80)
    SetBlipAsShortRange(aztecasmap, true)
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("Quartier Aztecas")
    EndTextCommandSetBlipName(aztecasmap)
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(Config.pos) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'aztecas' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'aztecas' then 
...
RMenu.Add('garageaztecas', 'main', RageUI.CreateMenu("Garage", "Garage aztecas"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('garageaztecas', 'main'), true, true, true, function() 
            ...
            Citizen.Wait(1)  
            spawnuniCar("primo")
            SetVehicleCustomPrimaryColour(vehicle, 0, 102, 204) 
            end
        end)
        ...
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'aztecas' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'aztecas' then    
                    ...
                    if IsControlJustPressed(1,51) then           
                        RageUI.Visible(RMenu:Get('garageaztecas', 'main'), not RageUI.Visible(RMenu:Get('garageaztecas', 'main')))
                    end   
                end
            end 
         end
...
function spawnuniCar(car)
    ...
    local plaque = "aztecas"..math.random(1,9)
    SetVehicleNumberPlateText(vehicle, plaque) 
    SetPedIntoVehicle(GetPlayerPed(-1),vehicle,-1)
    SetVehicleCustomPrimaryColour(vehicle, 0, 102, 204)
    SetVehicleCustomSecondaryColour(vehicle, 0, 102, 204)
...
RMenu.Add('coffreaztecas', 'main', RageUI.CreateMenu("Coffre", " "))
RMenu.Add('coffreaztecas', 'mettre', RageUI.CreateMenu("Coffre", " "))
RMenu.Add('coffreaztecas', 'enlever', RageUI.CreateMenu("Coffre", " "))

Citizen.CreateThread(function()
    while true do

        RageUI.IsVisible(RMenu:Get('coffreaztecas', 'main'), true, true, true, function()


            RageUI.ButtonWithStyle("Prendre objet", "Pour prendre un objet.", {RightLabel = "→"},true, function(Hovered, Active, Selected)
                if (Selected) then   
                RageUI.CloseAll()
                OpenGetStocksaztecasMenu()
                end
                end)
                RageUI.ButtonWithStyle("Déposer objet", "Pour déposer un objet.", {RightLabel = "→"},true, function(Hovered, Active, Selected)
                if (Selected) then   
                RageUI.CloseAll()
                OpenPutStocksaztecasMenu()
                end
                end)
...
Citizen.CreateThread(function()
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'aztecas' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'aztecas' then  
                    ...
                    if IsControlJustPressed(1,51) then
                        RageUI.Visible(RMenu:Get('coffreaztecas', 'main'), not RageUI.Visible(RMenu:Get('coffreaztecas', 'main')))
                    end   
                end
               end 
        end
...
function OpenGetStocksaztecasMenu()
    ESX.TriggerServerCallback('esx_aztecasjob:prendreitem', function(items)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
                ...
                if not count then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_aztecasjob:prendreitems', itemName, count)
...
function OpenPutStocksaztecasMenu()
    ESX.TriggerServerCallback('esx_aztecasjob:inventairejoueur', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
                ...
                if not count then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_aztecasjob:stockitem', itemName, count)
...
RMenu.Add('aztecasf6', 'main', RageUI.CreateMenu("aztecas", "Intéraction"))
...
Citizen.CreateThread(function()
    while true do
        ...
        RageUI.IsVisible(RMenu:Get('aztecasf6', 'main'), true, true, true, function()
        ...
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'aztecas' then  
                    ...
                    if IsControlJustPressed(0, 167) then
                        RageUI.Visible(RMenu:Get('aztecasf6', 'main'), not RageUI.Visible(RMenu:Get('aztecasf6', 'main')))
                end   
        end 
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'aztecas' then 
