...
Citizen.CreateThread(function()
    ...
    local madrazomap = AddBlipForCoord(1392.57, 1141.62, 21.1)

    SetBlipSprite(madrazomap, 429)
    SetBlipColour(madrazomap, 0)
    SetBlipScale(madrazomap, 0.80)
    SetBlipAsShortRange(madrazomap, true)
    BeginTextCommandSetBlipName('STRING')
    AddTextComponentString("Maison Cartel")
    EndTextCommandSetBlipName(madrazomap)
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for k,v in pairs(Config.pos) do
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'madrazo' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'madrazo' then 
...
RMenu.Add('garagemadrazo', 'main', RageUI.CreateMenu("Garage", "Garage madrazo"))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('garagemadrazo', 'main'), true, true, true, function() 
        ...
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'madrazo' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'madrazo' then    
                    ...
                    if IsControlJustPressed(1,51) then           
                        RageUI.Visible(RMenu:Get('garagemadrazo', 'main'), not RageUI.Visible(RMenu:Get('garagemadrazo', 'main')))
                    end   
                end
               end 
        end
...
function spawnuniCar(car)
    ...
    local plaque = "madrazo"..math.random(1,9)
    SetVehicleNumberPlateText(vehicle, plaque) 
    SetPedIntoVehicle(GetPlayerPed(-1),vehicle,-1)
    SetVehicleCustomPrimaryColour(vehicle, 253, 254, 255)
    SetVehicleCustomSecondaryColour(vehicle, 253, 254, 255)
...
RMenu.Add('armumadrazo', 'main', RageUI.CreateMenu("Armurerie", " "))

Citizen.CreateThread(function()
    while true do
        RageUI.IsVisible(RMenu:Get('armumadrazo', 'main'), true, true, true, function()   
            ...
            RageUI.ButtonWithStyle(v.nom, nil, {RightLabel = "~r~$"..v.prix},true, function(Hovered, Active, Selected)
                if (Selected) then   
                    TriggerServerEvent('armurerie:madrazo', v.arme, v.prix)
                end
            end)
        end
...
Citizen.CreateThread(function()
        while true do
		   ...
		   if ESX.PlayerData.job and ESX.PlayerData.job.name == 'madrazo' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'madrazo' then  
                    ...
                    if IsControlJustPressed(1,51) then
                        RageUI.Visible(RMenu:Get('armumadrazo', 'main'), not RageUI.Visible(RMenu:Get('armumadrazo', 'main')))
                    end   
                end
            end 
        end
...
RMenu.Add('coffremadrazo', 'main', RageUI.CreateMenu("Coffre", " "))

Citizen.CreateThread(function()
    while true do

        RageUI.IsVisible(RMenu:Get('coffremadrazo', 'main'), true, true, true, function()
...
Citizen.CreateThread(function()
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'madrazo' or ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'madrazo' then  
                    ...
                    if IsControlJustPressed(1,51) then
                        RageUI.Visible(RMenu:Get('coffremadrazo', 'main'), not RageUI.Visible(RMenu:Get('coffremadrazo', 'main')))
                    end   
                end
               end 
        end
...
function OpenGetStocksmadrazoMenu()
    ESX.TriggerServerCallback('esx_madrazojob:prendreitem', function(items)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_get_item_count', {
                ...
                if not count then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_madrazojob:prendreitems', itemName, count)
...
function OpenPutStocksmadrazoMenu()
    ESX.TriggerServerCallback('esx_madrazojob:inventairejoueur', function(inventory)
        local elements = {}
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'stocks_menu', {
            ...
            ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'stocks_menu_put_item_count', {
                ...
                if not count then
                    ...
                    menu2.close()
                    menu.close()
                    TriggerServerEvent('esx_madrazojob:stockitem', itemName, count)
...
RMenu.Add('madrazof6', 'main', RageUI.CreateMenu("Madrazo", "Intéraction"))
...
Citizen.CreateThread(function()
    while true do
        ...
        RageUI.IsVisible(RMenu:Get('madrazof6', 'main'), true, true, true, function()
        ...
        RageUI.ButtonWithStyle("Menotter/démenotter", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
            if (Selected) then
                ...
                local target_id = GetPlayerServerId(target)
                if closestPlayer ~= -1 and closestDistance <= 3.0 then 
                TriggerServerEvent('esx_madrazojob:handcuff', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
            end
            ...
            RageUI.ButtonWithStyle("Escorter", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
					...
					local target_id = GetPlayerServerId(target)
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_madrazojob:drag', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
            end
            ...
            RageUI.ButtonWithStyle("Mettre dans un véhicule", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
					...
					local target_id = GetPlayerServerId(target)
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_madrazojob:putInVehicle', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
                end
            ...
            RageUI.ButtonWithStyle("Sortir du véhicule", nil, {RightLabel = "→"}, true, function(Hovered, Active, Selected)
                if (Selected) then
					...
					local target_id = GetPlayerServerId(target)
					if closestPlayer ~= -1 and closestDistance <= 3.0 then
                TriggerServerEvent('esx_madrazojob:OutVehicle', GetPlayerServerId(closestPlayer))
            else
                ESX.ShowNotification('Peronne autour')
            end
            end
...
Citizen.CreateThread(function()
        while true do
            ...
            if ESX.PlayerData.job and ESX.PlayerData.job.name == 'madrazo' then  

                    if IsControlJustPressed(0, 167) then
                        RageUI.Visible(RMenu:Get('madrazof6', 'main'), not RageUI.Visible(RMenu:Get('madrazof6', 'main')))
                end   
        end 
    end
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if ESX.PlayerData.job2 and ESX.PlayerData.job2.name == 'madrazo' then 
	...
	ESX.TriggerServerCallback('esx_madrazojob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
			...
			if data.current.value then
				TriggerServerEvent('esx_madrazojob:confiscatePlayerItem', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount)
